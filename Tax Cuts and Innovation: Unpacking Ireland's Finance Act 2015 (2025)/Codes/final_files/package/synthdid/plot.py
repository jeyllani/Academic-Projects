...
class Plot(object):
    def plot(self, model="sdid", figsize=(15, 7), title=None):
        ...
        if model == "sdid":
            ...
            result["actual_y"].plot(
                ax=ax, color="blue", linewidth=1, label="Actual IRL", alpha=0.6
            )
            result["sdid"].plot(
                ax=ax, color="red", linewidth=1, label="Syntetic IRL", alpha=0.6
            )
            ax.plot(
            ...
            result["actual_y"].plot(
                ax=ax, color="green", linewidth=1, label="Actual IRL", alpha=0.6
            )
            result["sc"].plot(
                ax=ax, color="red", linewidth=1, label="Syntetic IRL", alpha=0.6
            )
            ...
            ax.axvline(
                x=self.post_term[0], # (self.pre_term[1] + self.post_term[0]) * 0.5,
                linewidth=1,
                linestyle="dashed",
                color="blue",
                alpha=0.5,
            ...
            ax.set_title(
                r'\textbf{Synthetic Control Method}', fontsize=14, fontname='Latin Modern Math', usetex=True
            ) #: tau {round( post_actural_treat - counterfuctual_post_treat,4)}"
            ax.xaxis.set_major_locator(plt.MaxNLocator(integer=True))
            ax.xaxis.set_minor_locator(plt.MultipleLocator(1))
            years = np.arange(self.Y_pre_c.index.min(), self.Y_post_c.index.max() + 1)
            ax.set_xticks(years)
            ...
            sns.despine()
            ax.set_xlabel(r'\textbf{Years}', fontsize=14, fontname='Latin Modern Math', usetex=True)
            ax.set_ylabel(r'\textbf{' + title + '}', fontsize=14, fontname='Latin Modern Math', usetex=True)
            legend = ax.legend()
            ...
            ax.set_title(
                f"Difference in Differences : tau {round( post_actural_treat - counterfuctual_post_treat,2)}"
            )
            ax.legend()
            plt.show()
